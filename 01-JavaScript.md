...
> 之前的数据类型，只能存储一个值(比如：Number/String。要想存储班级中所有学生的姓名，该如何存储？
...
- 形参和实参
  ...
  > 1. 形式参数：在声明一个函数的时候，为了函数的功能更加灵活，有些值是固定不了的，对于这些固定不了的值。我们可以给函数设置参数。这个参数没有具体的值，仅仅起到一个占位置的作用，通常称之为形式参数，也叫形参。
