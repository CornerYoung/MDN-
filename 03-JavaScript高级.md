...
> 由于 JavaScript 高级还是针对 JavaScript 语言本身的一个进阶学习，所以在开始之前先对以前所学过的 JavaScript 相关知识点做一个快速复习总结。
### JavaScript 是什么
...
- 异步和单线程
  ...
  + 异步和单线程
    ...
    3.执行最后一行，打印300
    4.待所有程序执行完毕，处于空闲状态时，会马上看有没有暂存起来的要执行
    5.发现暂存起来的setTimeout重的函数，进行执行
...
属性是对象的状态，方法是对象的行为（完成某种任务）。比如，可以把动物抽象为animal对象，使用“属性”记录具体是那一种动物，使用“方法”表示动物的某种行为（奔跑、捕猎、休息等等）。
...
假设要处理学生的成绩表，为了表示一个学生的成绩，面向过程的程序可以用一个对象表示：
...
如果采用面向对象的程序设计思想，首选思考的不是程序的执行流程，
...
Class 是一种抽象概念，比如定义的 Class——Student ，是指学生这个概念，
...
对于上面的写法固然没有问题，但是假如要生成两个 `person` 实例对象呢？
...
这样封装确实爽多了，通过工厂模式解决了创建多个相似对象代码冗余的问题，
...
注意到，`Person()` 中的代码与 `createPerson()` 有以下几点不同之处：
...
使用构造函数的好处不仅仅在于代码的简洁性，更重要的是可以识别对象的具体类型了。
...
了解了 **构造函数-实例-原型对象** 三者之间的关系后，接下来来解释一下为什么实例对象可以访问原型对象中的成员。
...
也就是说，在调用 `person1.sayName()` 的时候，会先后执行两次搜索：

- 首先，解析器会问：“实例 person1 有 sayName 属性吗？”答：“没有。
- ”然后，它继续搜索，再问：“ person1 的原型有 sayName 属性吗？”答：“有。
- ”于是，它就读取那个保存在原型对象中的函数。
- 当调用 person2.sayName() 时，将会重现相同的搜索过程，得到相同的结果。
...
注意到，前面例子中每添加一个属性和方法就要敲一遍 `Person.prototype` 。
...
在该示例中，将 `Person.prototype` 重置到了一个新的对象。
...
那了解了函数 this 指向的不同场景之后，有些情况下为了使用某种特定环境的 this 引用，
这时候时候就需要采用一些特殊手段来处理了，例如经常在定时器外部备份 this 引用，然后在定时器函数内部使用外部 this 的引用。
然而实际上对于这种做法的 JavaScript 为专门提供了一些函数方法用来帮更优雅的处理函数内部 this 指向问题。
